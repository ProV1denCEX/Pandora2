...
def get_factor(tick, window, quantile, freq):
    return PVMSpreadSubXPV(window, quantile, freq).set_output(transform="pandas").transform(tick)


def get_multi_factors(tick, windows, quantile, freq, n_job):
...
class PVMSpreadSubXPV(TickFeatureTemplate):
    col_required = [
